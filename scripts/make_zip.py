...
def  main():
  ...
  installer = "\installer\AmpMod Installer.exe"
   
  if demo:
    installer = "\installer\AmpMod Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\AmpMod manual.pdf" 
  ]
  ...
  ZIPNAME = "AmpMod-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "AmpMod-v" + FULLVERSIONSTR + "-win-demo.zip"
